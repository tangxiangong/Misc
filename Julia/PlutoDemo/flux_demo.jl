...
# ╔═╡ 923c48d0-61d0-11ee-25d2-87a151b40c4b
using PlutoUI, Flux, ChainPlots, Statistics, ProgressMeter

# ╔═╡ 25170ef1-46eb-4f49-bd75-02ecade5e05c
using Plots

# ╔═╡ 27b34411-22ba-497a-abe1-1eefe4cf39a8
begin
	noisy = rand(Float32, 2, 5_000)
	truth = [xor(col[1] > 0.5, col[2]>0.5) for col in eachcol(noisy)]
	nothing
end

# ╔═╡ 27e108b3-6c43-41fe-9a00-1cdb8e3d2b07
@bind N_epoch Slider(100:100:1_000, default=400,show_value=true)

# ╔═╡ 9d5c4d91-4085-4e38-9641-6a899b9828d0
begin
	model = Chain(
		Dense(2=>3, tanh),
		BatchNorm(3),
...
# ╔═╡ 25f87eb5-c089-4d9f-b216-2ec1a57242f4
begin
	result = model(noisy)
	accuracy = mean((result[1,:] .> 0.5) .== truth)
	md"The accuracy is $accuracy"
end

# ╔═╡ bd408817-6cb0-4899-aefc-85d9f4706a7d
begin
	p_true = scatter(noisy[1,:], noisy[2,:], zcolor=truth, title="True classifiaction")
	p_done = scatter(noisy[1, :], noisy[2,:], zcolor=result[1,:], title="Trained")
	plot(p_true, p_done, layout=(1,2), size=(600,330))
end

# ╔═╡ 5d13a990-4659-474d-a00a-b92e571a41bf
begin
	plot(losses;
...
[deps]
ChainPlots = "43761bbe-b505-42c3-b8bd-5d4ae40fd227"
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "20fee3f1efe98660c6b04b1f4b802ef00c1eb5cb"
...
[[deps.ChainPlots]]
deps = ["Flux", "Functors", "Graphs", "MetaGraphs", "Plots", "Random", "RecipesBase"]
git-tree-sha1 = "02db57a19a9e650bedae1b2fcc64f0e89125b801"
