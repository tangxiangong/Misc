...
md"请选择模拟路径长度 $(@bind T Slider(10:100, default=50, show_value=true))"

# ╔═╡ 7a9c15c6-831b-42c0-a79d-9e1d80e45480
begin
	if cls == "复合布朗运动"
		md"请选择参数大小 $(@bind p Slider(0.1:0.1:0.9, default=0.7, show_value=true))"
	elseif cls == "莱维运动"
		md"请选择参数大小 $(@bind p Slider(0.1:0.1:1.9, default=1.2, show_value=true))"
	else
		p = 2.0
		nothing
	end
...
begin
	function levy(length_t, param, τ=0.01)
		t = collect(0.0:τ:length_t)
		n = length(t)
		d = AlphaStable(param, 0.0, 1.0, 0.0)
		noise = @. τ^(1/param) * rand(d, n-1)
		x = cumsum([0.0;noise])
		return t, x
	end
	nothing
...
begin
	...
	function inv_subordinator(length_t, param, τ=0.01)
		t_, s_ = subordinator(length_t, param)
		count = 1
		while s_[end] < length_t
			t_, s_ = subordinator(2^count*length_t, param)
			count += 1
		end
		index = s_ .<= length_t
		t_temp = @view t_[index]
		s_temp = @view s_[index]
		n = length(t_temp)
		t = collect(0.0:τ:length_t)
		invₛ = zeros(Float64, length(t))
		index = t .<= s_temp[1]
		invₛ[index] .= t_temp[1]
		@inbounds for k in 1:n-1
			index = (t .> s_temp[k]) .&& (t .<= s_temp[k+1])
			invₛ[index] .= t_temp[k+1]
		end
		index = t .> s_temp[end]
		invₛ[index] .= t_[n+1]
		return t, invₛ
	end
	function subbrown(length_t, param, τ=0.01)
		t, s = inv_subordinator(length_t, param, τ)
		n = length(t)
		temp = diff(s)
		noise = zeros(Float64, n-1)
		@inbounds @simd for k in eachindex(noise)
			noise[k] = sqrt(temp[k]) * randn()
		end
		x = cumsum([0; noise])
		return t, x
	end
...
begin
	...
	if cls == "复合布朗运动"
		diffusion = "Subordinated Brownian motion"
	elseif cls == "莱维运动"
		diffusion = "Lévy motion"
	else
		diffusion = "Brownian motion"
	end
	plot(t, x, title=diffusion, label=false, grid=false)
	xlabel!(L"time $t$")
	ylabel!("Trajectory")
end
...
[compat]
AlphaStableDistributions = "~1.1.6"
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "b973102bfb891431dccd0e01b22e7144d7ca8a5b"
